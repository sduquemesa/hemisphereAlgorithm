...
bool isIsolated( Event &event, vector<int> index, double R ){
  ...
  for ( int i = 0; i < index.size(); i++){
    ...
    for ( int j = 0; j < event.size(); ++j ){
      ...
      if ( j == index[i] ) continue; // Avoid analysis the same particle
...
int main(int argc, char * argv[])
{
  ...
  event:
    ...
    for ( int i = 0; i < pythia.event.size(); ++i ){ // Particle loop
      ...
      if ( !(pythia.event[i].isFinal()) ) continue; // Only stable particles i.e. status >= 0
	...
	if (  pythia.event[i].pT() < 8.0 || abs( pythia.event[i].eta() ) >= 2.1 ) goto event; // Invalid event, discard. Goto next event.
	iE.push_back(i);
    ...
    /* === e-mu trigger === */
    if ( iMu.size() > 0 && iMup.size() > 0 && iE.size() > 0 && iEp.size() > 0 ) {
    ...
    if ( iE.size() > 0 && iEp.size() > 0 && iMup.size() == 0 ){ // Single electron
      cout << "single e" << endl; getchar();
      if ( pTelectrons[2] < 1 ) continue;
    }
    if ( iMu.size() > 0 && iMup.size() > 0 && iEp.size() == 0 ){ // Single muon
